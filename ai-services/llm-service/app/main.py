...
from .prompt import build_cv_prompt, build_fit_prompt, build_job_extraction_prompt
...
async def _call_llm(system_prompt: str, user_prompt: str, max_tokens: int) -> dict:
    try:
        ...
        if settings.remote_llm_url or settings.remote_llm_api_key or settings.gemma_api_key:
...
async def _handle_job_extract(payload: JobExtractionRequest) -> dict:
    system_prompt, user_prompt = build_job_extraction_prompt(payload.job)
    max_tokens = _max_tokens_for(payload.role)
    return await _call_llm(system_prompt, user_prompt, max_tokens)
...
@app.post("/v1/jobs/extract", response_model=dict)
async def extract_job(
    ...
    cached = _get_cached("job", payload.model_dump())
    if cached:
        return cached

    result = await _handle_job_extract(payload)
    _set_cache("job", payload.model_dump(), result)
    return result


@app.post("/v1/jobs/extract/async", response_model=TaskCreateResponse)
async def extract_job_async(
    ...
    cached = _get_cached("job", payload.model_dump())
    if cached:
        record = await _task_store.create(TaskType.job_extraction)
        await _task_store.update(record.task_id, status="succeeded", result=cached)
        return TaskCreateResponse(taskId=record.task_id, status="succeeded")
