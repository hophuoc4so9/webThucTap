...
export const cvApi = {
  ...
  suggestImprovementsAsync: async (
    id: number,
    userId?: number,
  ): Promise<{ taskId: string; status: string; result?: CvSuggestionResponse }> => {
    const res = await axiosClient.post<{ taskId: string; status: string; result?: CvSuggestionResponse }>(
      `cvs/${id}/suggestions/async`,
      { userId },
    );
    return res.data;
  },
  ...
  getNotifications: async (userId: number): Promise<any[]> => {
    const res = await axiosClient.get<any[]>(`cvs/notifications`, {
      params: { userId },
    });
    return res.data;
  },
  ...
  markNotificationRead: async (id: number): Promise<void> => {
    await axiosClient.put(`cvs/notifications/${id}/read`);
  },
