...
export function JobDetailDrawer({ job, onClose }: JobDetailDrawerProps) {
  ...
  const tagsRequirement = useMemo(() => {
    if (!job.tagsRequirement) return [];
    try {
      const parsed = JSON.parse(job.tagsRequirement);
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full z-50 w-full max-w-lg bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {job.aiAnalysisStatus === "succeeded" && tagsRequirement.length > 0 && (
            <div className="bg-purple-50/50 rounded-xl p-4 border border-purple-100/50 animate-in fade-in zoom-in duration-300">
              <h3 className="text-[11px] font-black text-purple-600 uppercase tracking-widest mb-3 flex items-center gap-2">
                <Sparkles size={14} className="animate-pulse" /> AI Verified Requirements
              </h3>
              <div className="flex flex-wrap gap-2">
                {tagsRequirement.map((tag, i) => (
                  <span key={i} className="px-2.5 py-1 bg-white text-purple-700 text-[10px] font-bold rounded-lg border border-purple-100 shadow-sm hover:scale-105 transition-transform cursor-default">
                    {tag}
                  </span>
                ))}
              </div>
