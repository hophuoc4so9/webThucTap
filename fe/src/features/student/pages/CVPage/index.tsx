...
export const CVPage = () => {
  ...
  const [analysisCvId, setAnalysisCvId] = useState<number | null>(null);
  ...
  useEffect(() => {
    const state = location.state as { saved?: boolean; savedType?: "create" | "update"; viewAnalysisForCvId?: number } | null;
    if (state?.saved) {
      showToast(state.savedType === "update" ? "Đã cập nhật CV" : "Đã tạo CV thành công");
      window.history.replaceState({}, "", window.location.pathname);
      setPage(1);
    }
    if (state?.viewAnalysisForCvId && userId) {
      // Small delay to ensure the page is rendered first
      setTimeout(() => {
        handleAnalyze(state.viewAnalysisForCvId!);
      }, 100);
      const newState = { ...state };
      delete newState.viewAnalysisForCvId;
      window.history.replaceState(newState, "", window.location.pathname);
    }
  }, [location.state, userId]); // eslint-disable-line
  ...
  const handleAnalyze = async (id: number) => {
    ...
    setAnalyzingId(id);
    try {
      const res = await cvApi.suggestImprovementsAsync(id, userId);
      if (res.status === "succeeded" && res.result) {
        setAnalysisResult(res.result);
        setAnalysisCvId(id);
      } else {
        showToast("Đang gửi yêu cầu phân tích vào hàng đợi AI... Bạn có thể tiếp tục công việc khác.");
      }
    } catch (e) {
  ...
  const handleCloseAnalysis = () => { setAnalysisResult(null); setAnalysisCvId(null); };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {analysisResult && (
        <>
          <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm" onClick={handleCloseAnalysis} />
          <div className="fixed inset-0 z-50 p-4 flex items-center justify-center pointer-events-none">
            <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col pointer-events-auto">
              {/* Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  {analysisCvId && (
                    <button
                      onClick={() => window.open(`/student/cv/${analysisCvId}/edit`, "_blank")}
                  ...
                  <button onClick={handleCloseAnalysis} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 transition-colors">
                    <X size={16} />
                  </button>
                </div>
              </div>
