...
function formatSalary(job: Job): string | null {
  // Nếu lương min và max đều bằng 0 hoặc chuỗi "0" thì coi là thỏa thuận
  if ((job.salaryMin === "0" || Number(job.salaryMin) === 0) && 
      (job.salaryMax === "0" || Number(job.salaryMax) === 0)) {
    return "Thỏa thuận";
  }
    ...
    (async () => {
      ...
      try {
        ...
        if (data.companyRef) {
          ...
          const found = await companyService.getCompanyById(data.companyId);
