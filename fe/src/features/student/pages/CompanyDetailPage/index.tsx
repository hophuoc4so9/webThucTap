  ...
  return (
    <div className="min-h-screen bg-blue-50/40">
      <div className="max-w-5xl mx-auto px-6 py-6">
        <Link
          to="/student/companies"
          className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-blue-500 mb-4 transition-colors"
        >
          <ChevronLeft size={16} /> Danh sách công ty
        </Link>

        {/* Banner + Identity */}
        <div className="bg-white rounded-xl border border-blue-100 overflow-hidden mb-5">
          <div className="relative h-44 bg-gradient-to-r from-blue-300 to-blue-500">
            {company.banner && (
              <img
                src={company.banner}
                alt=""
          ...
          <div className="px-6 pb-5">
            <div className="flex items-end gap-4 -mt-8 mb-3">
              <div className="w-20 h-20 rounded-xl bg-white shadow-md border border-blue-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                {company.logo ? (
                  <img
                    src={company.logo}
                    alt={company.name}
        ...
        <div className="flex flex-col lg:flex-row gap-5">
                    ...
                    {!jobsLoaded ? (
                      <div className="space-y-3">
                        {Array.from({ length: 3 }).map((_, i) => (
                          <div
                            key={i}
                            className="h-24 bg-blue-50 rounded-lg animate-pulse"
                          />
                        ))}
                      </div>
                    ) : jobs.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {jobs.map((job) => (
                          <Link
                            ...
                            className="block p-4 border border-blue-100 rounded-xl hover:border-blue-300 hover:shadow-sm transition-all group"
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <h4 className="font-medium text-sm text-gray-800 group-hover:text-blue-500 transition-colors">
                                  {job.title}
                                </h4>
                              ...
                              <ExternalLink
                                size={14}
                                className="text-gray-300 group-hover:text-blue-400 flex-shrink-0 mt-0.5"
                              />
                            </div>
                          </Link>
                        ))}
          ...
          <aside className="lg:w-64 flex-shrink-0 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {company.followers != null && (
                    <div className="flex flex-col items-center justify-center bg-blue-50 rounded-xl p-3 text-center">
                      <Heart size={16} className="text-blue-400 mb-1" />
                ...
                <div className="space-y-2">
                  {Object.entries(socialMedia).map(([platform, url]) => (
                    <a
                      ...
                      className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-blue-50 hover:bg-blue-100 text-gray-600 hover:text-blue-600 transition-colors group"
                    >
                      <span className="text-blue-300 group-hover:text-blue-500">
            ...
            <div className="bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl p-5 text-center shadow-sm">
              ...
              <button
                onClick={() => setTab("jobs")}
                className="w-full py-2 bg-white text-blue-500 rounded-lg text-xs font-semibold hover:bg-blue-50 transition-colors"
              >
                Xem việc làm
              </button>
            </div>
