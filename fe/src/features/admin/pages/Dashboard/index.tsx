...
import { skillAdminApi } from "@/features/admin/api/skillAdmin";
...
export function AdminDashboard() {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const [userStats, jobsRes, appsRes, companiesRes, skillsRes] = await Promise.allSettled([
        userApi.getStats(),
        jobService.getJobs({ page: 1, limit: 1 }),
        applicationApi.getAll({ limit: 1 }),
        companyApi.getAllAdmin({ status: "pending", limit: 1 }),
        skillAdminApi.getCandidates({ status: 'pending', limit: 1 }),
      ]);
      setStats({
        totalUsers:
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-amber-800">
                Có {stats.pendingSkills} kỹ năng mới cần duyệt
              </p>
              <p className="text-xs text-amber-600">
                Hệ thống AI vừa trích xuất kỹ năng mới, vui lòng duyệt để làm giàu từ điển.
              </p>
            </div>
          </div>
          <button
            onClick={() => navigate("/admin/skills/pending")}
