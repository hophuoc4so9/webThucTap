...
export class CvGatewayController {
  ...
  /** GET /cvs/notifications?userId=xxx */
  @Get("notifications")
  async getNotifications(@Query("userId", ParseIntPipe) userId: number) {
    try {
      return await firstValueFrom(
        this.cvClient.send("cv_get_notifications", { userId }),
      );
    } catch (err: any) {
      const { statusCode = 500, message = "Lỗi máy chủ" } =
        err?.error ?? err ?? {};
  ...
  /** PUT /cvs/notifications/:id/read */
  @Put("notifications/:id/read")
  async markNotificationRead(@Param("id", ParseIntPipe) id: number) {
    try {
      return await firstValueFrom(
        this.cvClient.send("cv_mark_notification_read", { id }),
      );
    } catch (err: any) {
      const { statusCode = 500, message = "Lỗi máy chủ" } =
        err?.error ?? err ?? {};
  ...
  /** GET /cvs/search — Tìm kiếm ứng viên (Dành cho nhà tuyển dụng) */
  @Get("search")
