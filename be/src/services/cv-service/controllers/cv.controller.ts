...
export class CvController {
  ...
  /** Lấy chi tiết CV công khai */
  @MessagePattern("cv_find_one_public")
  findOnePublic(@Payload() payload: { id: number }) {
    return this.cvService.findOnePublic(payload.id);
  }
  ...
  @MessagePattern("cv_get_notifications")
  getNotifications(@Payload() payload: { userId: number }) {
    return this.cvService.getNotifications(payload.userId);
  }

  @MessagePattern("cv_mark_notification_read")
  markNotificationRead(@Payload() payload: { id: number }) {
    return this.cvService.markNotificationRead(payload.id);
  }

  @MessagePattern("cv_search_candidates")
  searchCandidates(
    @Payload()
    payload: {
      jobId?: number;
    ...
    return this.cvService.searchCandidates(payload);
  }
}
