...
export class GemmaService {
  ...
  async suggestCvImprovements(
    ...
    const systemPrompt =
      "You are an expert recruitment AI. Your goal is to help fresh graduates and students optimize their CVs to stand out. " +
      "Focus on technical skills, academic projects, internships, and extracurricular activities. " +
      "Be a strict reviewer. Return ONLY valid JSON. No markdown.";
    const userPrompt = JSON.stringify(
      {
        ...
        rules: [
          ...
          "Be specific about what to improve (e.g., 'Add technology stack to project X').",
          "Ensure suggestions are actionable for a student or fresh graduate.",
        ],
        outputSchema: {
          score: "0-100",
          summary: "string",
          ...
          improvements: [
            {
              section: "summary|skills|experience|projects|education|general",
              issue: "string",
              suggestion: "string",
  ...
  private buildCvPayload(cv: any): Record<string, unknown> {
    return {
      ...
      gpa: cv.gpa || "",
      activities: this.parseStringArray(cv.activities),
      awards: this.parseStringArray(cv.awards),
  ...
  private normalizeCvSuggestion(data: Record<string, unknown>): CvSuggestionResponse {
    ...
    return {
      ...
      extractedSkills: this.stringArrayOr(data.extractedSkills, []),
      improvements,
  ...
  private normalizeResumeParse(data: Record<string, unknown>): ParsedResumeData {
    return {
      ...
      studentId: this.stringOr(data.studentId, ""),
      class: this.stringOr(data.class, ""),
      academicYear: this.stringOr(data.academicYear, ""),
      birthday: this.stringOr(data.birthday, ""),
      gender: this.stringOr(data.gender, ""),
      careerObjective: this.stringOr(data.careerObjective, ""),
      ...
      gpa: this.stringOr(data.gpa, ""),
      activities: this.stringArrayOr(data.activities, []),
      awards: this.stringArrayOr(data.awards, []),
